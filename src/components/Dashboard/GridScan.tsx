...
import styles from './GridScan.module.css';
...
function smoothDampVec2(current: THREE.Vector2, target: THREE.Vector2, currentVelocity: THREE.Vector2, smoothTime: number, maxSpeed: number, deltaTime: number) {
  ...
  const targetVec = current.clone().sub(change);
  const temp = currentVelocity.clone().addScaledVector(change, omega).multiplyScalar(deltaTime);
  currentVelocity.sub(temp.clone().multiplyScalar(omega));
  currentVelocity.multiplyScalar(exp);
  out.copy(targetVec.clone().add(change.add(temp).multiplyScalar(exp)));
...
export function GridScan({
  ...
  useEffect(() => {
    ...
    if (enablePost) {
      ...
      (bloom.blendMode.opacity as any).value = Math.max(0, bloomIntensity);
  ...
  useEffect(() => {
    ...
    if (bloomRef.current) {
      (bloomRef.current.blendMode.opacity as any).value = Math.max(0, bloomIntensity);
  ...
  // Webcam/face detection disabled for now - can be enabled later if needed
  useEffect(() => {
    if (!enableWebcam) {
    ...
    // Face detection would go here if needed
    setModelsReady(false);
    setUiFaceActive(false);
  }, [enableWebcam]);

  return (
    <div ref={containerRef} className={`${styles.gridscan}${className ? ` ${className}` : ''}`} style={style}>
      {showPreview && (
        <div className={styles.preview}>
          <video ref={videoRef} muted playsInline autoPlay className={styles.video} />
