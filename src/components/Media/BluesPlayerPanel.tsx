...
/**
 * Blues Player Panel Component
 *
 * Renders the Omega Blues Player interface with YouTube iframe integration,
 * waveform animation, and playback controls. Plays a fixed Dark Blues playlist.
 */

import React, { useState, useEffect, useRef } from "react";
import styles from "./BluesPlayerPanel.module.css";
...
export function BluesPlayerPanel() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("omega:bluesPlayerControl", handleControl as EventListener);
    window.addEventListener("omega:closeBluesPlayer", handleCloseEvent);

    return () => {
      clearTimeout(autoPlayTimer);
      window.removeEventListener("omega:bluesPlayerControl", handleControl as EventListener);
      window.removeEventListener("omega:closeBluesPlayer", handleCloseEvent);
  ...
  const handleClose = () => {
    ...
    if (typeof window !== "undefined") {
      window.dispatchEvent(
        new CustomEvent("omega:closeBluesPlayer", {
          detail: {},
        })
      );
    }
    ...
    <div className={styles.panel}>
      <div className={styles.header}>
        <div className={styles.headerLeft}>
          ...
          <h2 className={styles.title}>Omega Blues</h2>
        </div>
        <button
          className={styles.closeButton}
          onClick={handleClose}
          aria-label="Close Blues player"
        >
          ✕
        </button>
      </div>
      ...
      <div className={styles.content}>
        ...
        <div className={styles.trackInfo}>
          <div className={styles.trackTitle}>Dark Blues Playlist</div>
          <div className={styles.trackArtist}>Dark Blues Playlist</div>
        </div>
