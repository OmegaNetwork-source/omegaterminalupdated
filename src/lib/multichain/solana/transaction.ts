/**
 * Solana Transaction Handling Module
 * Handles transaction signing and sending for both Phantom and browser wallets
 */
...
/**
 * Send a transaction using Phantom or browser wallet
...
export async function sendTransaction(
  ...
  try {
    ...
    try {
      // Try deserializing as base64 (Jupiter format)
    ...
    console.error("[Solana Transaction] Error sending transaction:", error);
    return {
      signature: "",
...
async function sendWithPhantom(
  ...
  try {
    ...
    console.log(
      "[Solana Transaction] Transaction sent via Phantom:",
      signature
    );

    return { signature };
  } catch (error: any) {
    console.error("[Solana Transaction] Phantom error:", error);
...
/**
 * Send transaction using browser wallet (keypair)
 * @param transaction - Transaction or VersionedTransaction
...
async function sendWithBrowserWallet(
  ...
  try {
    ...
    console.log(
      "[Solana Transaction] Transaction sent via browser wallet:",
      signature
    );

    return { signature };
  } catch (error: any) {
    console.error("[Solana Transaction] Browser wallet error:", error);
    return {
      signature: "",
...
export async function confirmTransaction(
  ...
  try {
    ...
    console.log("[Solana Transaction] Transaction confirmed:", signature);
...
export async function getTransactionStatus(
  ...
  try {
    ...
    console.log("[Solana Transaction] Transaction status:", status);

    return status;
  } catch (error: any) {
    console.error("[Solana Transaction] Error getting status:", error);
    return null;
  }
}
