...
function handleVolume(context: CommandContext, args: string[]) {
  const volume = args[2];
  if (!volume) {
    const usageHtml = createUsageError("blues volume <0-100>", [
      "blues volume 50",
      "blues volume 75",
    ]);
...
function handleHelp(context: CommandContext, args: string[]) {
  ...
  context.log("", "output");
  const exampleHtml = createCommandLine("blues volume 75", "Set volume to 75%");
  ...
  handler: async (context: CommandContext, args: string[]) => {
    ...
    switch (subcommand) {
      ...
      default:
        ...
        const helpHtml = createCommandLine("blues help", "See available commands");
