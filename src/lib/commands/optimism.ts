/**
 * Optimism Network Commands
 * Network operations for Optimism
 */
...
const network = NETWORK_CONFIGS.optimism;

/**
 * Optimism command - Network operations for Optimism
 */
export const optimismCommand: Command = {
  name: "optimism",
  description: "Optimism network operations",
  usage: "optimism <balance|swap|help>",
  category: "network",
  handler: async (context: CommandContext, args: string[]) => {
    const subcommand = args[1]?.toLowerCase();

    if (!subcommand || subcommand === "help") {
      showOptimismHelp(context);
      return;
    }

    switch (subcommand) {
      case "balance":
        await getOptimismBalance(context);
        break;
      case "swap":
        if (args.length >= 5) {
          await handleOptimismSwap(context, args);
        } else {
          await showOptimismSwapInterface(context);
        }
        break;
      default:
...
function showOptimismHelp(context: CommandContext): void {
  const helpLines: string[] = [
    "optimism",
    "",
    "Optimism network operations",
    "",
    "→ Usage: optimism <balance|swap|help>",
    "",
    "═ Commands ═",
    ...
    "optimism balance",
    "",
    `Check ${network.nativeTokenSymbol} balance on Optimism`,
    "",
    "→ Usage: optimism balance",
    "",
    "optimism swap",
    "",
    "Swap tokens on Optimism using Uniswap & PancakeSwap",
    "",
    "→ Usage: optimism swap <fromToken> <toToken> <amount>",
    `  Example: optimism swap ETH USDC 1`,
    "  Example: optimism swap USDC OP 100",
    "",
  ];
  ...
  helpLines.forEach((line) => {
    if (line.trim() === "") {
      ...
      const isCommand = line.startsWith("optimism ") && line.length < 50;
      if (isCommand) {
...
async function getOptimismBalance(context: CommandContext): Promise<void> {
...
/**
 * Show interactive Optimism swap interface
 */
async function showOptimismSwapInterface(context: CommandContext): Promise<void> {
  if (!context.wallet?.address) {
    context.log("❌ Please connect your wallet first", "error");
    const helpHtml = `
      <div style="margin: 8px 0;">
        ${createCommandLine("connect", "Connect your wallet")}
        ${createCommandLine("optimism help", "See Optimism commands")}
      </div>
    `;
    context.logHtml(helpHtml);
    return;
  ...
  const html = createNetworkSwapInterfaceHTML(network, "optimism");
  context.logHtml(html);

  if (typeof window !== "undefined") {
    setTimeout(() => {
      setupNetworkSwapInterface(context, network, "optimism", "optimism", async (ctx, fromToken, toToken, amount) => {
        await handleOptimismSwapDirect(ctx, fromToken, toToken, amount);
      });
    }, 100);
  }
}

async function handleOptimismSwapDirect(
...
async function handleOptimismSwap(
  context: CommandContext,
  args: string[]
): Promise<void> {
  ...
  if (!fromToken || !toToken || !amount) {
    const usageHtml = createUsageError(`optimism swap <fromToken> <toToken> <amount>`, [
      "optimism swap ETH USDC 1",
      "optimism swap USDC OP 100",
    ]);
  ...
  await handleOptimismSwapDirect(context, fromToken, toToken, amount);
}

export const optimismCommands: Command[] = [optimismCommand];
