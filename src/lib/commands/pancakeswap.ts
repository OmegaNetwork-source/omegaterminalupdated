 ...
 * Multi-chain PancakeSwap integration for token swaps
 */
...
const SUPPORTED_NETWORKS = [
  { name: "BNB Smart Chain", value: "bnb", chainId: 56 },
...
/**
 * Common tokens across networks for PancakeSwap
 */
...
/**
 * PancakeSwap command - Multi-chain token swaps
 */
export const pancakeswapCommand: Command = {
  name: "pancakeswap",
  description: "PancakeSwap token swaps across multiple networks",
  usage: "pancakeswap [network] [swap|help]",
  category: "network",
  handler: async (context: CommandContext, args: string[]) => {
    ...
    if (!network || network === "help") {
      showPancakeSwapHelp(context);
      return;
    }

    if (subcommand === "help" || !subcommand) {
      showPancakeSwapNetworkHelp(context, network);
      return;
    }
    ...
    if (subcommand === "swap") {
      ...
      if (args.length >= 6) {
        ...
        await showPancakeSwapSwapInterface(context, network);
      }
    } else {
...
function showPancakeSwapHelp(context: CommandContext): void {
  const helpHtml = `
    <div style="
    ...
    ">
      ...
      <div style="color: var(--palette-text, #e0e0e0); margin-bottom: 16px;">
        PancakeSwap token swaps across multiple networks
      </div>
...
function showPancakeSwapNetworkHelp(context: CommandContext, network: string): void {
  ...
  const helpHtml = `
    ...
    ">
      <div style="
        ...
        border-radius: 4px;
      ">
        ═══ PANCAKESWAP - ${networkInfo.name.toUpperCase()} ═══
      </div>
      <div style="color: var(--palette-text, #e0e0e0); margin-bottom: 16px;">
        PancakeSwap token swaps on ${networkInfo.name}
      </div>
      ...
      <div style="color: var(--palette-text, #e0e0e0); margin-top: 8px; font-size: 0.9em;">
        Example: pancakeswap ${network} swap BNB USDC 1
      </div>
      <div style="color: var(--palette-text, #e0e0e0); margin-top: 8px; font-size: 0.9em;">
        Or: pancakeswap ${network} swap (opens interactive interface)
      </div>
    </div>
  `;
  context.logHtml(helpHtml);
...
/**
 * Show interactive PancakeSwap swap interface
 */
async function showPancakeSwapSwapInterface(context: CommandContext, network: string): Promise<void> {
  if (!context.wallet?.address) {
    ...
    const helpHtml = `
      <div style="margin: 8px 0;">
        ...
        ${createCommandLine("pancakeswap help", "See PancakeSwap commands")}
      </div>
    `;
    context.logHtml(helpHtml);
    return;
  ...
  const tokens = COMMON_TOKENS[network] || COMMON_TOKENS.bnb;
  ...
  context.log(`🥞 PancakeSwap Swap Interface - ${networkInfo.name}`, "info");
  ...
  const html = `
    ...
    ">
      <h3 style="margin: 0 0 15px 0; color: var(--palette-primary, #00d4ff); font-size: 18px;">
        🥞 PancakeSwap Token Swap - ${networkInfo.name}
      </h3>
      ...
      <div style="margin-bottom: 15px;">
        ...
        <div style="position: relative;">
          <input type="text" id="pancakeswapFromSearch_${network}" placeholder="Search tokens..." 
          ...
          <div id="pancakeswapFromList_${network}" style="position: absolute; top: 100%; left: 0; right: 0; max-height: 200px; overflow-y: auto; background: color-mix(in srgb, var(--palette-primary, #00bcf2) 10%, transparent); border: 1px solid var(--palette-border, rgba(0, 212, 255, 0.3)); border-top: none; z-index: 1000; display: none; border-radius: 0 0 4px 4px;"></div>
        </div>
        <input type="hidden" id="pancakeswapFromTokenAddress_${network}" value="${nativeToken.address}">
        <div id="pancakeswapFromTokenDisplay_${network}" style="margin-top: 5px; padding: 8px; background: color-mix(in srgb, var(--palette-primary, #00bcf2) 10%, transparent); border-radius: 4px; color: var(--palette-primary, #00d4ff); border-left: 3px solid var(--palette-primary, #00d4ff); font-family: 'Courier New', monospace;">
      ...
      <div style="margin-bottom: 15px;">
        ...
        <div style="position: relative;">
          <input type="text" id="pancakeswapToSearch_${network}" placeholder="Search tokens..." 
            style="width: 100%; padding: 8px; background: color-mix(in srgb, var(--palette-primary, #00bcf2) 5%, transparent); color: var(--palette-text, #ffffff); border: 1px solid var(--palette-border, rgba(0, 212, 255, 0.3)); border-radius: 4px; box-sizing: border-box; cursor: text; font-family: 'Courier New', monospace;" autocomplete="off">
          <div id="pancakeswapToList_${network}" style="position: absolute; top: 100%; left: 0; right: 0; max-height: 200px; overflow-y: auto; background: color-mix(in srgb, var(--palette-primary, #00bcf2) 10%, transparent); border: 1px solid var(--palette-border, rgba(0, 212, 255, 0.3)); border-top: none; z-index: 1000; display: none; border-radius: 0 0 4px 4px;"></div>
        </div>
        <input type="hidden" id="pancakeswapToTokenAddress_${network}" value="">
        <div id="pancakeswapToTokenDisplay_${network}" style="margin-top: 5px; padding: 8px; background: color-mix(in srgb, var(--palette-primary, #00bcf2) 10%, transparent); border-radius: 4px; color: color-mix(in srgb, var(--palette-text, #ffffff) 70%, transparent); border-left: 3px solid var(--palette-border, rgba(0, 212, 255, 0.3)); font-family: 'Courier New', monospace;">Select a token</div>
      </div>
      
      <!-- Amount input -->
      <div style="margin-bottom: 15px;">
        <label style="color: var(--palette-text, #e0e0e0); display: block; margin-bottom: 5px; font-weight: 600;">Amount:</label>
        <input type="text" id="pancakeswapSwapAmount_${network}" placeholder="1.0" 
      ...
      <div style="display: flex; gap: 10px; margin-bottom: 15px;">
        <button id="pancakeswapQuoteBtn_${network}" style="flex: 1; padding: 12px; background: linear-gradient(135deg, var(--palette-primary, #00bcf2), var(--palette-primary, #00d4ff)); color: white; border: none; border-radius: 5px; font-weight: bold; cursor: pointer; font-family: 'Courier New', monospace; transition: all 0.2s ease;" onmouseover="this.style.opacity = '0.9'; this.style.transform = 'scale(1.02)';" onmouseout="this.style.opacity = '1'; this.style.transform = 'scale(1)';">
          Get Quote
        </button>
        <button id="pancakeswapSwapBtn_${network}" style="flex: 1; padding: 12px; background: linear-gradient(135deg, var(--palette-secondary, #00ff88), var(--palette-primary, #00d4ff)); color: white; border: none; border-radius: 5px; font-weight: bold; cursor: pointer; font-family: 'Courier New', monospace; transition: all 0.2s ease;" onmouseover="this.style.opacity = '0.9'; this.style.transform = 'scale(1.02)';" onmouseout="this.style.opacity = '1'; this.style.transform = 'scale(1)';">
          Execute Swap
        </button>
      </div>
      ...
      <div style="background: color-mix(in srgb, var(--palette-primary, #00bcf2) 10%, transparent); padding: 10px; border-radius: 4px; border-left: 3px solid var(--palette-primary, #00d4ff);">
        <small style="color: var(--palette-text, #e0e0e0); font-size: 11px;">
          🥞 Start typing to search for tokens. Powered by PancakeSwap on ${networkInfo.name}.
        </small>
      </div>
    </div>
  `;
  ...
  if (typeof window !== "undefined" && tokens) {
    setTimeout(() => {
      setupPancakeSwapSwapInterface(context, network, tokens);
    }, 100);
  }
}

/**
 * Setup PancakeSwap swap interface event handlers
 */
function setupPancakeSwapSwapInterface(context: CommandContext, network: string, tokens: Array<{ symbol: string; name: string; address: string; verified: boolean }>): void {
  const fromSearch = document.getElementById(`pancakeswapFromSearch_${network}`) as HTMLInputElement;
  const toSearch = document.getElementById(`pancakeswapToSearch_${network}`) as HTMLInputElement;
  const fromList = document.getElementById(`pancakeswapFromList_${network}`);
  const toList = document.getElementById(`pancakeswapToList_${network}`);
  const quoteBtn = document.getElementById(`pancakeswapQuoteBtn_${network}`);
  const swapBtn = document.getElementById(`pancakeswapSwapBtn_${network}`);
  ...
  fromSearch.addEventListener("input", async (e) => {
    ...
    fromSearchTimeout = setTimeout(async () => {
      if (fromList) {
        ...
        const results = await searchPancakeSwapTokens(query, networkInfo.chainId);
        fromList.innerHTML = "";
        ...
        if (results.length > 0) {
          results.forEach((token) => {
            ...
            const verificationBadge = token.verified
              ...
              : '<span style="color: var(--palette-warning, #ffa502); font-size: 12px;">⚠️ UNVERIFIED</span>';
            ...
            const logoHtml = token.logoURI 
              ? `<img src="${token.logoURI}" alt="${token.symbol}" style="width: 24px; height: 24px; border-radius: 50%; margin-right: 8px;" onerror="this.style.display='none'">`
              : '';
            ...
            item.addEventListener("click", (e) => {
              ...
              const fromTokenAddressInput = document.getElementById(
                `pancakeswapFromTokenAddress_${network}`
              ) as HTMLInputElement;
              if (fromTokenAddressInput) {
                fromTokenAddressInput.value = token.address;
              }

              const fromTokenDisplay = document.getElementById(`pancakeswapFromTokenDisplay_${network}`);
              if (fromTokenDisplay) {
          ...
          results.forEach((token) => {
            ...
            item.addEventListener("click", (e) => {
              ...
              const toTokenAddressInput = document.getElementById(
                `pancakeswapToTokenAddress_${network}`
              ) as HTMLInputElement;
              if (toTokenAddressInput) {
                toTokenAddressInput.value = token.address;
              }

              const toTokenDisplay = document.getElementById(`pancakeswapToTokenDisplay_${network}`);
              if (toTokenDisplay) {
  ...
  if (quoteBtn) {
    quoteBtn.addEventListener("click", async () => {
      const fromToken = (
        document.getElementById(`pancakeswapFromTokenAddress_${network}`) as HTMLInputElement
      )?.value;
      const toToken = (
        document.getElementById(`pancakeswapToTokenAddress_${network}`) as HTMLInputElement
      )?.value;
      const amount = (document.getElementById(`pancakeswapSwapAmount_${network}`) as HTMLInputElement)
        ?.value;
      ...
      context.log("📊 Getting PancakeSwap quote...", "info");
      await handlePancakeSwapSwap(context, ["", "", network, "swap", fromToken, toToken, amount]);
    });
  }
  ...
  if (swapBtn) {
    swapBtn.addEventListener("click", async () => {
      ...
      context.log("💱 Executing PancakeSwap swap...", "info");
      await handlePancakeSwapSwap(context, ["", "", network, "swap", fromToken, toToken, amount]);
    });
  }
...
async function handlePancakeSwapSwap(
  ...
  if (!network || !fromToken || !toToken || !amount) {
    const usageHtml = createUsageError("pancakeswap <network> swap <fromToken> <toToken> <amount>", [
      "pancakeswap bnb swap BNB USDC 1",
      "pancakeswap ethereum swap ETH USDC 0.5",
    ]);
  ...
  if (!fromTokenInfo && !isNativeToken(fromToken) && fromToken.startsWith("0x")) {
    const searchResults = await searchPancakeSwapTokens(fromToken, networkInfo.chainId);
  ...
  if (!toTokenInfo && !isNativeToken(toToken) && toToken.startsWith("0x")) {
    const searchResults = await searchPancakeSwapTokens(toToken, networkInfo.chainId);
  ...
  if (!isNativeToken(fromTokenAddress)) {
    ...
    const tokenInfo = await searchPancakeSwapTokens(fromSymbol, networkInfo.chainId);
  ...
  context.log("💱 Preparing PancakeSwap transaction...", "info");
  ...
  try {
    ...
    if (!isNativeToken(toTokenAddress)) {
      const tokenInfo = await searchPancakeSwapTokens(toSymbol, networkInfo.chainId);
    ...
    const txParams = await buildPancakeSwapTransaction({
      tokenIn: fromTokenAddress,
      tokenOut: toTokenAddress,
      amountIn: amountInWei,
      recipient: context.wallet.address,
...
export const pancakeswapCommands: Command[] = [pancakeswapCommand];
