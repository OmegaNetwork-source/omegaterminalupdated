...
export const EXPLOSION_TYPES = {
  ...
  blast: {
    ...
    draw(explosion, foreground) {
      const {x, y, cr, r, osc} = explosion;
      const f = cycle(explosion.cr, 6) % 2 === 0 ? 55 : 110;
      osc.frequency.setValueAtTime(f, audio.currentTime);
      // Use enhanced explosion rendering
      enhancedGfx.drawEnhancedExplosion(foreground, x, y, cr, cr, r);
    },
    stop(explosion) {
      const {osc} = explosion;
      osc.stop();
  ...
  dirt: {
    ...
    draw(explosion, foreground, terrain) {
      ...
      // Use enhanced dirt explosion
      enhancedGfx.drawEnhancedDirt(foreground, x, y, cr, terrain.color);
    },
    clip(explosion, terrain) {
      const {x, y, cr} = explosion;
