...
export function initMenu() {
  // Remove existing menu canvas if it exists
  const existingMenuCanvas = document.querySelector('canvas[data-menu]');
  if (existingMenuCanvas) {
    existingMenuCanvas.remove();
  }
  ...
  menuCanvas.style.display = 'none'; // Hidden by default - React menu handles it
...
function updateButtons() {
  ...
  if (menuState === 'main') {
    buttons = [
      ...
      {id: 'gamemodes', text: 'GAME MODES', y: 200, action: () => {
        // Dispatch custom event to trigger React UI
        const event = new CustomEvent('openGameModes');
        window.dispatchEvent(event);
        return null;
      }},
      {id: 'leaderboard', text: 'LEADERBOARD', y: 240, action: () => {
        // Dispatch custom event to trigger React UI
        const event = new CustomEvent('openLeaderboard');
        window.dispatchEvent(event);
        return null;
      }},
      {id: 'settings', text: 'SETTINGS', y: 280, action: () => {menuState = 'settings'; selectedButton = 0; updateButtons(); return null;}},
      {id: 'wallet', text: walletAddress ? `${walletAddress.slice(0, 6)}...${walletAddress.slice(-4)}` : 'CONNECT WALLET', y: 320, action: () => {menuState = 'wallet'; selectedButton = 0; updateButtons(); return null;}},
      {id: 'howtoplay', text: 'HOW TO PLAY', y: 360, action: () => {menuState = 'howtoplay'; selectedButton = 0; updateButtons(); return null;}},
    ];
  } else if (menuState === 'gamemodes') {
    buttons = [
...
export function drawMenu() {
  ...
  if (menuState === 'main') {
    ...
    drawGameModesMenu(ctx);
  } else if (menuState === 'leaderboard') {
    drawLeaderboardMenu(ctx);
  } else if (menuState === 'settings') {
...
function drawGameModesMenu(ctx) {
  drawText(ctx, 'GAME MODES', W/2, 60, 'gold', 'center');
  drawText(ctx, 'Use React UI buttons', W/2, 100, 'white', 'center');
  drawText(ctx, 'in top-right corner', W/2, 120, 'white', 'center');
  drawText(ctx, 'or click GAME MODES', W/2, 140, 'white', 'center');
  drawText(ctx, 'button to configure', W/2, 160, 'white', 'center');
...
function drawLeaderboardMenu(ctx) {
  drawText(ctx, 'LEADERBOARD', W/2, 60, 'gold', 'center');
  drawText(ctx, 'Use React UI button', W/2, 100, 'white', 'center');
  drawText(ctx, 'in top-right corner', W/2, 120, 'white', 'center');
  drawText(ctx, 'to view full stats', W/2, 140, 'white', 'center');
...
export function hideMenu() {
  ...
  // Trigger React state update if available
  if (typeof window !== 'undefined' && window.notifyReactState) {
    window.notifyReactState();
  }
}
