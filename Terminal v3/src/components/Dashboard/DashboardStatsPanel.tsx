...
import { GioPridePanel } from "@/components/Effects/GioPridePanel";
...
export function DashboardStatsPanel(): JSX.Element | null {
  ...
  return (
    <aside ref={statsPanelRef} className={styles.statsPanel} onClick={handleStatsPanelClick}>
      ...
      {/* Gio Pride Panel - Shows when Gio theme is active, replaces SystemOverview */}
      {currentTheme === "gio" && !companion.isPanelOpen ? (
        <GioPridePanel />
      ) : (
        /* System Overview - Hidden when companion is open or Gio theme is active */
        !companion.isPanelOpen && (
